...
RegisterInterface::RegisterInterface(Client *data) : Fl_Double_Window(315, 166, "Register Window") {
    register_ui::client = data;
...
RegisterInterface::~RegisterInterface(){
    ...
    delete(repeat_password_input);
    delete(register_button);
}


void RegisterInterface::RegisterButtonCallback(Fl_Widget*, void*) {
    ...
    else if (username.size() <= 4 || username.size() > 16)
...
void RegisterInterface::CloseRegisterWindowCallback(Fl_Widget *, void *) {
    register_ui::client->interface_manager->ShowWindow(Login);
}