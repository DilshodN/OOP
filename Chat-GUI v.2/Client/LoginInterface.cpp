...
LoginInterface::LoginInterface(Client* data) : Fl_Double_Window(242, 120, "Login Window") {
    log_ui::client = data;
...
void LoginInterface::LoginButtonCallback(Fl_Widget *, void *) {
    ...
    if (username.size() <= 4 || username.size() > 16)
        fl_alert("Username length must be between four and sixteen characters.");
    else if (password.size() <= 6 || username.size() > 32)
        fl_alert("Password length must be between six and thirty-two characters.");
    else log_ui::client->Login(username, password);
}

void LoginInterface::OpenRegisterWindowButtonCallback(Fl_Widget *, void *) {
    log_ui::client->interface_manager->ShowWindow(Register);
}

void LoginInterface::CloseLoginWindowCallback(Fl_Widget *, void *) {
    log_ui::client->interface_manager->ShowWindow(Address);
}
