...
void Server::ProcessPacket(sf::TcpSocket* client, sf::Packet packet) {
    ...
    switch (type) {
        ...
        case (LOGIN | REQUEST): {
            logl("Login request from " << username << " and password " << data << " request type "
                                       << std::to_string(type));
        ...
        case (REGISTER | REQUEST): {
            logl("Register request from " << username << " and password " << data << " request type "
                                          << std::to_string(type));
            Data result;
            if (!database.GetQueryResult("SELECT username FROM users WHERE username = '" + username + "' LIMIT 1",
                                         result))
                logl("aqui!");
            if (result.rows == 0 && database.ExecuteQuery(
                    "INSERT INTO users (username, password) VALUES ('" + username + "', '" + data + "')")) {
