...
TEST(Algebra, Inverse) {
	{
		...
		auto im = m.inverse();
		EXPECT_EQ(im, i_expect)
			<< "error when finding the inverse matrix for: " << m;
	}
	...
	for (auto dim : dims) {
		...
		auto m = diagonal(dim, 0, 1);
		auto im = m.inverse();
		EXPECT_EQ(m * im, Matrix::identity(dim, dim)) 
			<< "error when finding the inverse matrix for: " << m 
			<< " result: " << im;
	}
}
...
TEST(Methods, Resize) {
    ...
    Matrix matrix1 (5, 1);
    for(size_t i = 0; i < matrix.rows(); i++)
        for(size_t j = 0; i < matrix.cols(); j++)
            ASSERT_NO_THROW(matrix.coeffRef(i, j) = i);
}
